...
/**
 * @brief json decode utility alowwing to decode string encoded in ISO-8859-1
 */
class JsonDecoder {
    public static function json_decode($str, $as_array, $text_encoding) {
        $decoded_str = json_decode($str, $as_array);
        if ($text_encoding === TextEncoding::ISO88591) {
            $decoded_str = $as_array ? JsonDecoder::array_utf8_decode($decoded_str) : JsonDecoder::object_utf8_decode($decoded_str);
        }
        return $decoded_str;
    }

    private static function object_utf8_decode($object) {
        $new_object = clone $object;
        foreach ($object as $key => $value) {
            if (is_string($key)) {
                $new_key = utf8_decode($key);
                unset($new_object->$key);

                if (is_string($value)) {
                    $new_object->$new_key = utf8_decode($value);
                } elseif (is_object($value)) {
                    $new_object->$new_key = JsonDecoder::object_utf8_decode($value);
                } elseif (is_array($value)) {
                    $new_object->$new_key = JsonDecoder::array_utf8_decode($value);
                } else {
                ...
                if (is_string($value)) {
                    $new_object->$key = utf8_decode($value);
                } elseif (is_object($value)) {
                    $new_object->$key = JsonDecoder::object_utf8_decode($value);
                } elseif (is_array($value)) {
                    $new_object->$key = JsonDecoder::array_utf8_decode($value);
                }
                else {
    ...
    private static function array_utf8_decode($array) {
        $new_array = $array;
        foreach ($array as $key => $value) {
            if (is_string($key)) {
                ...
                if (is_string($value)) {
                    $new_array[$new_key] = utf8_decode($value);
                } elseif (is_object($value)) {
                    $new_array[$new_key] = JsonDecoder::object_utf8_decode($value);
                } elseif (is_array($value)) {
                    $new_array[$new_key] = JsonDecoderis::array_utf8_decode($value);
                } else {
                ...
                if (is_string($value)) {
                    $new_array[$key] = utf8_decode($value);
                } elseif (is_object($value)) {
                    $new_array[$key] = JsonDecoder::object_utf8_decode($value);
                } elseif (is_array($value)) {
                    $new_array[$key] = JsonDecoder::array_utf8_decode($value);
                }
                else {
                    $new_array[$key] = $value;
                }
...
abstract class BasicEnum {
     ...
     */
    private static function is_valid_value($instance, $value)
    {
     ...
     */
    protected static function check_val($instance, $value, $msg=null)
    {
        if (! self::is_valid_value($instance, $value)) {
            if (is_null($msg)) {
                $msg = 'Invalid value: ';
            }
